...
@section('admin_content')
  ...
  @endif
    <div class="form-group row">
      <div id="">
        <a id="" href="{{url('admin/create-discount')}}" type="button" class="btn btn-primary" style="float: right;" title="Add Discount">Add Discount</a>&nbsp;&nbsp;
      </div>
    </div>
    @php
      $superSuperAdmin = 1;
      ...
      <tbody>
        @if(count($coursePayments) > 0)
          ...
          <tr>
            ...
            @endif
            <td>
              <a href="{{url('admin/discount')}}/{{$coursePayment->id}}/edit" ><img src="{{asset('images/edit1.png')}}" width='30' height='30'/>
                </a>
            </td>
            @if($superAdmin == $loginUser->type)
            <td>
              ...
              <form id="deleteDiscount_{{$coursePayment->id}}" action="{{url('admin/delete-discount')}}" method="POST" style="display: none;">
                  {{ csrf_field() }}
                  {{ method_field('DELETE') }}
          ...
          @endforeach
        @else
          <tr><td colspan="8">No discount is created.</td></tr>
        @endif
      </tbody>
    </table>
  </div>
